...
# Keep only these two genera
VALID_GENUS = {"Octopodichnus", "Paleohelcura"}


def get_label_from_txt(txt_path: Path):
    """Return the genus (string) for the first valid class in the label, or None if not valid."""
...
def collect_all_image_to_roi():
    """
    Build a mapping from base image name -> list of (roi_image_path, genus),
    using labels from train/val/test label folders.
    """
...
def split_image_to_rois(img_to_rois, ratio=(0.6, 0.2, 0.2)):
    """
    Split by original image (not ROI) to avoid leakage.
    ratio = (train, val, test) and should sum to 1.0.
    """
    ...
    # Assign the remainder to test to ensure full coverage
...
def save_split_by_img(img_to_rois, train_imgs, val_imgs, test_imgs):
    """
    Save ROI images into OUTPUT_ROOT/{train,val,test}/{genus}/, counting per genus.
    """
    ...
    for split, img_set in [("train", train_imgs), ("val", val_imgs), ("test", test_imgs)]:
        for base_name in img_set:
            for roi_path, genus in img_to_rois[base_name]:
                out_dir = OUTPUT_ROOT / split / genus  # genus is the class folder
                out_dir.mkdir(parents=True, exist_ok=True)
                dst = out_dir / roi_path.name
                shutil.copy2(roi_path, dst)
                stats[split][genus] += 1

    # Pretty print per-split genus counts
...
if __name__ == "__main__":
    ...
    print("\nDataset has been split by genus and saved under:", OUTPUT_ROOT)
